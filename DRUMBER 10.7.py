...
class DraggableBPM(ctk.CTkFrame):
    ...
    def _on_press(self, e):
        self._last_y = e.y_root
        self._bpm_at_press = self.app.engine.bpm

    def _on_release(self, e):
        if self.app.engine.bpm != getattr(self, '_bpm_at_press', self.app.engine.bpm):
            self.prev_bpm = self._bpm_at_press

    def _on_right_click(self, e):
        if hasattr(self, 'prev_bpm'):
            curr = self.app.engine.bpm
            self.app.save_state()
            self.set_val(self.prev_bpm)
            self.prev_bpm = curr
    ...
    def _on_wheel(self, e):
        now = time.time()
        if not hasattr(self, '_last_wheel_time') or now - self._last_wheel_time > 0.5:
            self.prev_bpm = self.app.engine.bpm
...
class NumpyJunoChorus:
    ...
    def process(self, audio, sr, rate_hz, depth_ms, phase_offset, mix, bpm, bpm_sync):
        ...
        if bpm_sync and bpm > 0:
            bps = bpm / 60.0
            mults =[8, 6, 4, 3, 2, 1, 0.75, 0.5, 0.25, 0.125]
...
class DrumChannel:
    def __init__(self, name, engine_ref):
        ...
        for _ in range(4): self.pattern_settings.append({'vol':0.8, 'pan':0.0, 'pitch':0, 's_start':0, 's_end':250, 'fx_sends':[0.0, 0.0, 0.0], 'euclid_k':0, 'fade_in':1.0, 'fade_out':100.0, 'smpl_idx':0})
        self.vol, self.pan, self.pitch, self.sample_start, self.sample_end, self.fade_in, self.fade_out, self.euclid_k = 0.8, 0.0, 0, 0, 250, 1.0, 100.0, 0
        self.fx_sends = [0.0, 0.0, 0.0]
        self.m_vol, self.m_pan, self.m_pitch, self.m_sample_start, self.m_sample_end, self.m_fade_in, self.m_fade_out, self.m_euclid_k = 0.8, 0.0, 0, 0, 250, 1.0, 100.0, 0
...
class AudioEngine:
    def __init__(self):
        ...
        self.steps, self.current_pattern, self.swing = 16, 0, 0
        self.global_vol, self.global_pitch = 0.8, 0
        self.master_fx_sends = [0.0, 0.0, 0.0]
        self.m_global_vol, self.m_global_pitch, self.m_swing = 0.8, 0, 0
        ...
        # Audio de-clicking engine properties
        self.stutter_vol = 0.0
        self.bg_vol = 1.0
    ...
    def audio_callback(self, outdata, frames, time_info, status):
        ...
        # DSP Volume Envelope Generation to prevent stutter click/pops
        fade_step = 1.0 / max(1, int(0.003 * SAMPLE_RATE))
        play_bg = getattr(self, 'play_bg_during_stutter', True) and not getattr(self, 'stutter_mute_bg', False)
        
        if getattr(self, 'stutter_active', False):
            target_s_vol = 0.707 if play_bg else 1.0
            target_b_vol = 0.707 if play_bg else 0.0
        else:
            target_s_vol = 0.0
            target_b_vol = 1.0

        s_vols = np.full(frames, target_s_vol, dtype=np.float32)
        b_vols = np.full(frames, target_b_vol, dtype=np.float32)
        
        if self.stutter_vol != target_s_vol:
            diff = target_s_vol - self.stutter_vol
            direction = np.sign(diff)
            steps = min(frames, int(abs(diff) / fade_step))
            if steps > 0:
                s_vols[:steps] = self.stutter_vol + direction * fade_step * np.arange(1, steps + 1)
            self.stutter_vol = s_vols[steps-1] if steps > 0 else self.stutter_vol
            if steps < frames:
                self.stutter_vol = target_s_vol
                s_vols[steps:] = target_s_vol

        if self.bg_vol != target_b_vol:
            diff = target_b_vol - self.bg_vol
            direction = np.sign(diff)
            steps = min(frames, int(abs(diff) / fade_step))
            if steps > 0:
                b_vols[:steps] = self.bg_vol + direction * fade_step * np.arange(1, steps + 1)
            self.bg_vol = b_vols[steps-1] if steps > 0 else self.bg_vol
            if steps < frames:
                self.bg_vol = target_b_vol
                b_vols[steps:] = target_b_vol

        # Stutter execution
        if (getattr(self, 'stutter_active', False) or self.stutter_vol > 0.0) and getattr(self, 'stutter_buffer', None) is not None:
            frames_left = frames
            out_pos = 0
            stutter_out = np.zeros_like(final_sig)
            
            while frames_left > 0:
                if self.stutter_recorded < self.stutter_len and getattr(self, 'stutter_active', False):
            ...
            final_sig = final_sig * b_vols.reshape(-1, 1) + stutter_out * s_vols.reshape(-1, 1)
        else:
            final_sig = final_sig * b_vols.reshape(-1, 1)
...
class DrumMachineApp(ctk.CTk):
    def __init__(self, engine):
        ...
        self.title("Drumber v10.7"); self.geometry("1450x1075"); ctk.set_appearance_mode("Dark"); self.configure(fg_color="#0A0A0A") 
        ...
        self.widget_to_btn_action = {}
        self.drag_action = None
        self.drag_toggled_channels = set()
        ...
        self.tips_pool =[
            ...
            "Randomize Sample: Left-click to pick a new sample for all unlocked tracks. Hold to reset. Right-click to undo.",
            "Randomize Euclidean: Left-click to apply new euclidean rhythms for all unlocked tracks. Hold to reset. Right-click to undo.",
            "Randomize Sample Length: Left-click to randomize start/end points. Hold to reset. Right-click to undo.",
            "Randomize Fades: Left-click to randomize fade in/out. Hold to reset. Right-click to undo.",
            "Randomize Pan: Left-click to randomize panning. Hold to reset. Right-click to undo.",
            "Randomize All Parameters: Left-click to randomize everything globally. Hold to reset. Right-click to undo.",
            "Unlock All: Left-click to unlock all channels. Right-click to undo.",
            "Unmute All: Left-click to unmute all channels. Right-click to undo.",
            "Unsolo All: Left-click to unsolo all channels. Right-click to undo.",
            "Unreverse All: Left-click to un-reverse all channels. Right-click to undo.",
            "Global Undo: Left-click to revert the last global action.",
            "Reset All: Left-click to reset all channels and FX to default settings.",
            "Mute Channel: Left-click and drag to mute/unmute multiple tracks.",
            "Solo Channel: Left-click and drag to solo multiple tracks.",
            "Reverse Sample: Left-click and drag to reverse multiple tracks.",
            "Juno Chorus: BPM Sync Rate"
        ]
        self.tip_queue =[]
        ...
        self.qwert_states = {'q': 0, 'w': 0, 'e': 0, 'r': 0, 't': 0}
        ...
        # Keyboard Bindings for App Functions
        self.bind_all("1", lambda e: self.set_pattern_from_key(1))
        self.bind_all("2", lambda e: self.set_pattern_from_key(2))
        self.bind_all("3", lambda e: self.set_pattern_from_key(3))
        self.bind_all("4", lambda e: self.set_pattern_from_key(4))
        self.bind_all("<Tab>", self.cycle_pattern_key)
        
        self.bind_all("<KeyPress-s>", self.on_s_press)
        self.bind_all("<KeyRelease-s>", self.on_s_release)
        self.bind_all("<KeyPress-S>", self.on_s_press)
        self.bind_all("<KeyRelease-S>", self.on_s_release)
        
        self.bind_all("<F1>", lambda e: self.set_stutter_div(0))
        self.bind_all("<F2>", lambda e: self.set_stutter_div(2))
        self.bind_all("<F3>", lambda e: self.set_stutter_div(4))
        self.bind_all("<F4>", lambda e: self.set_stutter_div(6))
        self.bind_all("<F5>", lambda e: self.set_stutter_div(8))
        self.bind_all("<F6>", lambda e: self.toggle_triplet())
        
        for k in ['q', 'w', 'e', 'r', 't']:
            self.bind_all(f"<KeyPress-{k}>", lambda e, key=k: self.on_qwert_key(key))
            self.bind_all(f"<KeyPress-{k.upper()}>", lambda e, key=k: self.on_qwert_key(key))
    ...
    def on_global_release(self, e):
        self.drag_action = None
        if hasattr(self, 'drag_toggled_channels'):
            self.drag_toggled_channels.clear()

    def set_pattern_from_key(self, num):
        focused = self.focus_get()
        if focused and ("entry" in focused.winfo_class().lower() or "dialog" in focused.winfo_class().lower()): return
        self.pat_var.set(str(num))
        self.change_pattern(str(num))

    def cycle_pattern_key(self, e):
        focused = self.focus_get()
        if focused and ("entry" in focused.winfo_class().lower() or "dialog" in focused.winfo_class().lower()): return
        next_pat = ((self.engine.current_pattern + 1) % 4) + 1
        self.pat_var.set(str(next_pat))
        self.change_pattern(str(next_pat))
        return "break"

    def on_s_press(self, e):
        focused = self.focus_get()
        if focused and ("entry" in focused.winfo_class().lower() or "dialog" in focused.winfo_class().lower()): return
        if not getattr(self, '_s_pressed', False):
            self._s_pressed = True
            self._on_stutter_press(None, False)

    def on_s_release(self, e):
        if getattr(self, '_s_pressed', False):
            self._s_pressed = False
            self._on_stutter_release(None)

    def on_qwert_key(self, key):
        focused = self.focus_get()
        if focused and ("entry" in focused.winfo_class().lower() or "dialog" in focused.winfo_class().lower()): return
        
        state = self.qwert_states[key]
        is_right_click = (state == 1)
        
        if key == 'q':
            self.undo_unlock_all() if is_right_click else self.unlock_all()
        elif key == 'w':
            self.undo_unmute_all() if is_right_click else self.unmute_all()
        elif key == 'e':
            self.undo_unsolo_all() if is_right_click else self.unsolo_all()
        elif key == 'r':
            self.undo_unreverse_all() if is_right_click else self.unreverse_all()
        elif key == 't':
            self.undo()
            
        self.qwert_states[key] = 1 - state

    def show_tip(self, text):
        if not self.show_tips_var.get(): return
    ...
    def tips_loop(self):
        ...
        if not self.show_tips_var.get() and hasattr(self, 'lbl_tips'):
            if self.lbl_tips.cget("text") != "":
                self.lbl_tips.configure(text="")
    ...
    def on_btn_press(self, action, ch_idx):
        self.drag_action = action
        self.drag_toggled_channels = {ch_idx}
        ch = self.engine.channels[ch_idx]
        btn = self.ch_ui_refs[ch.name][f'btn_{action}']
        if action == 'mute': self.toggle_mute(ch, btn)
        elif action == 'solo': self.toggle_solo(ch, btn)
        elif action == 'lock': self.toggle_lock(ch, btn)
        elif action == 'rev': self.toggle_rev(ch, btn)

    def register_btn_action(self, btn, action, ch_idx):
        if not hasattr(self, 'widget_to_btn_action'):
            self.widget_to_btn_action = {}
        self.widget_to_btn_action[str(btn)] = (action, ch_idx)
        btn.bind("<ButtonPress-1>", lambda e, a=action, idx=ch_idx: self.on_btn_press(a, idx))
        
        if hasattr(btn, '_canvas'):
            self.widget_to_btn_action[str(btn._canvas)] = (action, ch_idx)
            btn._canvas.bind("<ButtonPress-1>", lambda e, a=action, idx=ch_idx: self.on_btn_press(a, idx))
        if hasattr(btn, '_text_label') and btn._text_label:
            self.widget_to_btn_action[str(btn._text_label)] = (action, ch_idx)
            btn._text_label.bind("<ButtonPress-1>", lambda e, a=action, idx=ch_idx: self.on_btn_press(a, idx))
    ...
    def build_ui(self):
        ...
        self.add_global_knob(global_controls, "Swing", 0, 100, 0, lambda v: setattr(self.engine, 'swing', v), "gl:swing", is_int=True)
        ...
        self.pat_var = ctk.StringVar(value="1"); ctk.CTkSegmentedButton(pat_frame, values=["1", "2", "3", "4"], font=APP_FONT, variable=self.pat_var, command=self.change_pattern, width=80).pack(side="right")
        ...
        btn_1_4 = ctk.CTkButton(r1, text="1/4", width=34, height=32, font=("Trebuchet MS", 9), command=lambda: self.set_stutter_div(0))
        btn_1_4.pack(side="left", padx=1)
        btn_1_8 = ctk.CTkButton(r1, text="1/8", width=34, height=32, font=("Trebuchet MS", 9), command=lambda: self.set_stutter_div(2))
        btn_1_8.pack(side="left", padx=1)
        btn_1_16 = ctk.CTkButton(r1, text="1/16", width=34, height=32, font=("Trebuchet MS", 9), command=lambda: self.set_stutter_div(4))
        btn_1_16.pack(side="left", padx=1)

        btn_1_32 = ctk.CTkButton(r2, text="1/32", width=34, height=32, font=("Trebuchet MS", 9), command=lambda: self.set_stutter_div(6))
        btn_1_32.pack(side="left", padx=1)
        btn_1_64 = ctk.CTkButton(r2, text="1/64", width=34, height=32, font=("Trebuchet MS", 9), command=lambda: self.set_stutter_div(8))
        btn_1_64.pack(side="left", padx=1)

        btn_x3 = ctk.CTkButton(stutter_ctrl_container, text="X3", width=20, height=65, font=("Trebuchet MS", 9, "bold"), command=self.toggle_triplet)
        ...
        for w in [self.btn_stutter, stutter_ctrl_container, btn_1_4, btn_1_8, btn_1_16, btn_1_32, btn_1_64, btn_x3, divs_f, r1, r2]:
        ...
        rand_outer.pack(side="left", fill="y", padx=5)
        
        lbl_f_rand = ctk.CTkFrame(rand_outer, fg_color="transparent")
        lbl_f_rand.pack(side="top", pady=0)
        ...
        rand_frame = ctk.CTkFrame(rand_outer, fg_color="transparent"); rand_frame.pack(side="top", fill="both", expand=True)
        
        def mk_rnd(txt, key, col=None, text_col=None, width=48, font=APP_FONT):
            b = ctk.CTkButton(rand_frame, text=txt, width=width, font=font)
            if col: b.configure(fg_color=col)
            if text_col: b.configure(text_color=text_col)
            
            b.pack(side="left", fill="y", expand=(width==48), padx=1)
            
            def on_press(e, k=key, btn=b):
                btn._pressed = True
                btn._reset_triggered = False
                def trigger_reset():
                    if getattr(btn, '_pressed', False):
                        btn._reset_triggered = True
                        self.reset_rand(k)
                btn._timer_id = self.after(1200, trigger_reset)

            def on_release(e, k=key, btn=b):
                btn._pressed = False
                if hasattr(btn, '_timer_id'):
                    self.after_cancel(btn._timer_id)
                if not getattr(btn, '_reset_triggered', False):
                    self.do_global_rand(k)
        ...
        mk_rnd("Smpl", 'Cyc', col="#D35400", text_col="#FFFFFF") 
        mk_rnd("Euc", 'Euc') 
        mk_rnd("Len", 'Sam') 
        mk_rnd("Fad", 'Fad') 
        mk_rnd("Pan", 'Pan') 
        mk_rnd("ALL", 'ALL', col="#A93226", width=36, font=("Trebuchet MS", 10))
        
        # Global Reset Controls
        reset_outer = ctk.CTkFrame(global_controls, fg_color="transparent")
        reset_outer.pack(side="right", fill="y", padx=5)
        ctk.CTkLabel(reset_outer, text="GLOBAL CHANNEL OPTION RESET/TOGGLE", font=("Trebuchet MS", 10, "bold")).pack(side="top", pady=0)
        act_f = ctk.CTkFrame(reset_outer, fg_color="transparent"); act_f.pack(side="top", fill="both", expand=True)
        
        btn_unl = ctk.CTkButton(act_f, text="UNLOCK", width=45, font=APP_FONT_BOLD, command=self.unlock_all, fg_color="#8E44AD"); btn_unl.bind("<Button-3>", self.undo_unsolo_all); btn_unl.pack(side="left", fill="y", expand=True, padx=1)
        btn_unm = ctk.CTkButton(act_f, text="UNMUTE", width=45, font=APP_FONT_BOLD, command=self.unmute_all, fg_color="#922B21"); btn_unm.bind("<Button-3>", self.undo_unmute_all); btn_unm.pack(side="left", fill="y", expand=True, padx=1)
        btn_uns = ctk.CTkButton(act_f, text="UNSOLO", width=45, font=APP_FONT_BOLD, command=self.unsolo_all, fg_color="#9C640C"); btn_uns.bind("<Button-3>", self.undo_unsolo_all); btn_uns.pack(side="left", fill="y", expand=True, padx=1)
        btn_unrev = ctk.CTkButton(act_f, text="UNREV", width=45, font=APP_FONT_BOLD, command=self.unreverse_all, fg_color="#7D3C98"); btn_unrev.bind("<Button-3>", self.undo_unreverse_all); btn_unrev.pack(side="left", fill="y", expand=True, padx=1)
        btn_undo = ctk.CTkButton(act_f, text="UNDO", width=45, font=APP_FONT_BOLD, command=self.undo, fg_color="#1E8449"); btn_undo.pack(side="left", fill="y", expand=True, padx=1)
        btn_all = ctk.CTkButton(act_f, text="ALL", width=33, font=("Trebuchet MS", 10, "bold"), command=self.reset_all_params, fg_color="#6B1D1D"); btn_all.pack(side="left", fill="y", expand=False, padx=1)
        ...
        for i, ch in enumerate(self.engine.channels):
            ...
            btn_frame = ctk.CTkFrame(col, fg_color="transparent"); btn_frame.pack(fill="x", padx=4, pady=1)
            
            btn_lock = ctk.CTkButton(btn_frame, text="🔓", width=10, height=22, font=("Arial", 12), fg_color="#333333", hover_color="#555555")
            btn_lock.pack(side="left", expand=True, fill="x", padx=1); self.ch_ui_refs[ch.name]['btn_lock'] = btn_lock
            
            btn_mute = ctk.CTkButton(btn_frame, text="M", width=10, font=("Trebuchet MS", 10, "bold"), fg_color="#444444")
            btn_mute.pack(side="left", expand=True, fill="x", padx=1); self.ch_ui_refs[ch.name]['btn_mute'] = btn_mute
            self.widget_to_lfo_target[str(btn_mute)] = (f"ch:{i}:mute", f"{ch.name} Mute")
            
            btn_solo = ctk.CTkButton(btn_frame, text="S", width=10, font=("Trebuchet MS", 10, "bold"), fg_color="#444444") 
            btn_solo.pack(side="left", expand=True, fill="x", padx=1); self.ch_ui_refs[ch.name]['btn_solo'] = btn_solo
            self.widget_to_lfo_target[str(btn_solo)] = (f"ch:{i}:solo", f"{ch.name} Solo")
            
            btn_rev = ctk.CTkButton(btn_frame, text="R", width=10, font=("Trebuchet MS", 10, "bold"), fg_color="#444444")
            btn_rev.pack(side="left", expand=True, fill="x", padx=1); self.ch_ui_refs[ch.name]['btn_rev'] = btn_rev
            self.widget_to_lfo_target[str(btn_rev)] = (f"ch:{i}:reverse", f"{ch.name} Reverse")
            
            btn_cycl = ctk.CTkButton(col, text="Sample", height=22, border_width=1, border_color="#FFFFFF", font=("Trebuchet MS", 10, "bold"), fg_color="#D35400", hover_color="#E67E22", text_color="#FFFFFF")
            btn_cycl.bind("<ButtonPress-1>", lambda e, c=ch, b=btn_cycl: self._on_cyc_press(c, b)); btn_cycl.bind("<ButtonRelease-1>", lambda e, c=ch, b=btn_cycl: self._on_cyc_release(c, b)); btn_cycl.bind("<Button-3>", lambda e, c=ch: self.cycle_drumb(c, False)); 
            btn_cycl.pack(pady=(4, 2), anchor="center")
            self.widget_to_lfo_target[str(btn_cycl)] = (f"ch:{i}:cyc", f"{ch.name} Sample")

            self.register_btn_action(btn_mute, 'mute', i)
            self.register_btn_action(btn_lock, 'lock', i)
            self.register_btn_action(btn_solo, 'solo', i)
            self.register_btn_action(btn_rev, 'rev', i)
    ...
    def build_fx_rack_ui(self, i):
        ...
        knob_send = CTkKnob(send_f, width=44, height=44, from_=0, to=1, command=on_send_change, progress_color=color)
        ...
        center_f = ctk.CTkFrame(header_f, fg_color="transparent")
        center_f.pack(expand=True)
        
        btn_grab = ctk.CTkLabel(center_f, text="≡", font=("Arial", 16, "bold"), text_color="#888888", cursor="fleur")
        btn_grab.pack(side="left", padx=(0, 5))
        
        lbl_title = ctk.CTkLabel(center_f, text=f"FX{i+1}", font=APP_FONT_BOLD, text_color=color, height=20)
        lbl_title.pack(side="left", padx=(0, 10))
        
        opt = ctk.CTkOptionMenu(center_f, values=list(FX_DEFS.keys()), font=APP_FONT, width=100, height=24, command=lambda v, idx=i: self.on_fx_type_change(idx, v))
        opt.set(self.engine.fx_buses[i].fx_type); opt.pack(side="left")

        def start_drag(e, idx):
            self.drag_fx_start_y = e.y_root
            self.drag_fx_idx = idx
            self.drop_indicator = ctk.CTkFrame(self.fx_racks_inner, height=4, fg_color="#F39C12")
            self.current_target_idx = idx

        def on_drag(e):
            y = self.fx_racks_inner.winfo_pointery()
            children = [w for w in self.fx_racks_inner.winfo_children() if w != getattr(self, 'drop_indicator', None)]
            target = len(children)
            for k, child in enumerate(children):
                if y < child.winfo_rooty() + child.winfo_height()/2:
                    target = k
                    break
            if target != getattr(self, 'current_target_idx', -1):
                self.current_target_idx = target
                for c in children: c.pack_forget()
                if hasattr(self, 'drop_indicator'): self.drop_indicator.pack_forget()
                for k, c in enumerate(children):
                    if k == target and hasattr(self, 'drop_indicator'):
                        self.drop_indicator.pack(fill="x", padx=5, pady=2)
                    c.pack(fill="x", padx=5, pady=3)
                if target == len(children) and hasattr(self, 'drop_indicator'):
                    self.drop_indicator.pack(fill="x", padx=5, pady=2)

        def end_drag(e):
            if hasattr(self, 'drop_indicator'):
                self.drop_indicator.destroy()
                del self.drop_indicator
            if hasattr(self, 'current_target_idx'):
                tgt = self.current_target_idx
                if tgt > self.drag_fx_idx:
                    tgt -= 1
                if tgt != self.drag_fx_idx and 0 <= tgt < len(self.engine.fx_buses):
                    self.move_fx_bus(self.drag_fx_idx, tgt)
                del self.current_target_idx

        btn_grab.bind("<ButtonPress-1>", lambda e, idx=i: start_drag(e, idx))
        btn_grab.bind("<B1-Motion>", on_drag)
        btn_grab.bind("<ButtonRelease-1>", end_drag)
    ...
    def move_fx_bus(self, from_idx, to_idx):
        self.save_state()
        
        bus = self.engine.fx_buses.pop(from_idx)
        self.engine.fx_buses.insert(to_idx, bus)
        
        send = self.engine.master_fx_sends.pop(from_idx)
        self.engine.master_fx_sends.insert(to_idx, send)
        
        m_send = self.engine.m_master_fx_sends.pop(from_idx)
        self.engine.m_master_fx_sends.insert(to_idx, m_send)
        
        for ch in self.engine.channels:
            ch_send = ch.fx_sends.pop(from_idx)
            ch.fx_sends.insert(to_idx, ch_send)
            
            ch_m_send = ch.m_fx_sends.pop(from_idx)
            ch.m_fx_sends.insert(to_idx, ch_m_send)
            
            for p in ch.pattern_settings:
                if 'fx_sends' in p:
                    p_send = p['fx_sends'].pop(from_idx)
                    p['fx_sends'].insert(to_idx, p_send)
                    
        for v in self.engine.active_voices:
            if len(v['fx_sends']) > from_idx:
                v_send = v['fx_sends'].pop(from_idx)
                v['fx_sends'].insert(to_idx, v_send)
                
        for child in self.fx_racks_inner.winfo_children():
            child.destroy()
        self.fx_ui_refs.clear()
        
        for i in range(len(self.engine.fx_buses)):
            if f"FX{i+1}" in self.g_ui_refs: del self.g_ui_refs[f"FX{i+1}"]
            if f"FX{i+1}" in self.g_lbl_refs: del self.g_lbl_refs[f"FX{i+1}"]
            
        for i in range(len(self.engine.fx_buses)):
            self.build_fx_rack_ui(i)
            
        for i, ch in enumerate(self.engine.channels):
            for child in self.ch_ui_refs[ch.name]['fx_container'].winfo_children():
                child.destroy()
            self.ch_ui_refs[ch.name]['fx_knobs'] =[]
            for j in range(len(self.engine.fx_buses)):
                self.build_ch_fx_knob(ch, i, j)
    ...
    def open_options(self):
        ...
        w = ctk.CTkToplevel(self); w.title("Options"); w.geometry("800x480"); w.transient(self)
        self.options_window = w
        
        main_frame = ctk.CTkFrame(w, fg_color="transparent")
        main_frame.pack(fill="both", expand=True)
        
        left_f = ctk.CTkFrame(main_frame, fg_color="transparent")
        left_f.pack(side="left", fill="both", expand=True, padx=10)
        
        right_f = ctk.CTkFrame(main_frame, fg_color="#1A1A1A", corner_radius=8)
        right_f.pack(side="right", fill="both", expand=True, padx=10, pady=10)
        
        ctk.CTkLabel(left_f, text="Settings", font=APP_FONT_BOLD).pack(pady=10)
        
        ctk.CTkLabel(left_f, text="Scan Method:", font=APP_FONT).pack(pady=(10, 0))
        ctk.CTkSegmentedButton(left_f, values=["Lazy", "Normal", "Focused"], variable=self.scan_method_var).pack(pady=5)
        
        ctk.CTkCheckBox(left_f, text="Auto-detect BPM on load", font=APP_FONT, variable=self.auto_detect_bpm).pack(pady=5)
        ctk.CTkCheckBox(left_f, text="Show tips by the settings icon", font=APP_FONT, variable=self.show_tips_var).pack(pady=5)
        ...
        if out_devs:
            ctk.CTkLabel(left_f, text="Audio Output Device", font=APP_FONT).pack(pady=(10, 0))
            ...
            ctk.CTkOptionMenu(left_f, values=out_devs, variable=self.audio_dev_var, font=APP_FONT, command=self.change_audio_device).pack(pady=5)
            
        if HAS_MIDO:
            ctk.CTkLabel(left_f, text="MIDI Sync Settings", font=APP_FONT_BOLD).pack(pady=(10, 0))
            ...
            if midi_in_devs:
                ...
                ctk.CTkOptionMenu(left_f, values=opts, variable=self.midi_dev_var, font=APP_FONT, command=self.change_midi_device).pack(pady=5)
                
                self.midi_sync_var = ctk.BooleanVar(value=getattr(self.engine, 'midi_sync', False))
                ctk.CTkCheckBox(left_f, text="Enable External MIDI Sync", font=APP_FONT, variable=self.midi_sync_var, command=self.toggle_midi_sync).pack(pady=5)
            else:
                ctk.CTkLabel(left_f, text="No MIDI input devices found.", font=APP_FONT).pack(pady=5)
        else:
            ctk.CTkLabel(left_f, text="MIDI sync unavailable (mido not installed)", font=APP_FONT).pack(pady=10)

        # Right Panel Shortcuts
        ctk.CTkLabel(right_f, text="Keyboard Shortcuts", font=APP_FONT_BOLD, text_color="#2ECC71").pack(pady=(15, 5))
        
        shortcuts =[
            ("Space", "Play / Pause"),
            ("1, 2, 3, 4", "Select Pattern 1-4"),
            ("Tab", "Cycle Pattern"),
            ("s / S (Hold)", "Stutter"),
            ("F1 - F5", "Stutter Div (1/4 - 1/64)"),
            ("F6", "Toggle Stutter Triplet"),
            ("q, w, e, r", "Unlock, Unmute, Unsolo, Unreverse All"),
            ("t", "Global Undo"),
            ("Ctrl (Hold)", "Adjust All Channels via Slider"),
        ]
        ...
        grid_f.grid_columnconfigure(1, weight=2)
        
        for idx, (key, desc) in enumerate(shortcuts):
            ctk.CTkLabel(grid_f, text=key, font=("Trebuchet MS", 12, "bold"), text_color="#3498DB", anchor="e").grid(row=idx, column=0, sticky="e", padx=(10, 15), pady=8)
            ctk.CTkLabel(grid_f, text=desc, font=("Trebuchet MS", 12), anchor="w").grid(row=idx, column=1, sticky="w", padx=(0, 10), pady=8)
    ...
    def sync_ui_to_engine(self):
        ...
        self.bpm_widget.set_val(self.engine.bpm); self.pat_var.set(str(self.engine.current_pattern + 1)); self.step_var.set(str(self.engine.steps))
        ...
        for ch in self.engine.channels:
            ...
            r['btn_mute'].configure(fg_color="#922B21" if ch.mute else "#444444")
            r['btn_lock'].configure(text="🔒", text_color="#8E44AD") if ch.locked else r['btn_lock'].configure(text="🔓", text_color="#FFFFFF")
            r['btn_solo'].configure(fg_color="#9C640C" if ch.solo else "#444444")
            r['btn_rev'].configure(fg_color="#7D3C98" if ch.reverse else "#444444")
    ...
    def reset_all_params(self, e=None):
        ...
        self.engine.apply_state({
            'bpm': 75, 'steps': 16, 'pat': 0, 'swing': 0, 'gvol': 0.8, 'gpitch': 0, 'm_fx_sends': [0.0]*len(self.engine.fx_buses), 'stutter_div_idx': 4,
            'ch':[{'idx': 0, 'mute': False, 'solo_l': False, 'locked': False, 'rev': False, 'vol': 0.8, 'pan': 0.0, 'pitch': 0, 's_start': 0, 's_end': 250, 'fx_sends': [0.0]*len(self.engine.fx_buses), 'euc': 0, 'seq': [[False]*32 for _ in range(4)], 'ps':[{'vol':0.8, 'pan':0.0, 'pitch':0, 's_start':0, 's_end':250, 'fx_sends':[0.0]*len(self.engine.fx_buses), 'euclid_k':0, 'fade_in':1.0, 'fade_out':100.0, 'smpl_idx':0} for _ in range(4)]} for _ in range(8)],
    ...
    def rescan_single_channel(self, ch, btn, restore_text="Sample"):
    ...
    def process_audio_file(self, filepath, is_rescan=False):
        ...
        try:
            ...
            def _update_gui():
                ...
                self.title(f"Drumber v10.7 - {os.path.basename(filepath)}")
    ...
    def undo_unlock_all(self, e=None):
        ...
        for ch in self.engine.channels:
            if ch.name in state:
                ...
                btn.configure(text="🔒", text_color="#8E44AD") if ch.locked else btn.configure(text="🔓", text_color="#FFFFFF")
    ...
    def undo_unsolo_all(self, e=None):
        ...
        for ch in self.engine.channels:
            if ch.name in state:
                ...
                if ch.solo: self.ch_ui_refs[ch.name]['btn_solo'].configure(fg_color="#9C640C")
    ...
    def undo_unmute_all(self, e=None):
        ...
        for ch in self.engine.channels:
            if ch.name in state:
                ...
                self.ch_ui_refs[ch.name]['btn_mute'].configure(fg_color="#922B21" if ch.mute else "#444444")
    ...
    def undo_unreverse_all(self, e=None):
        ...
        for ch in self.engine.channels:
            if ch.name in state:
                ...
                self.ch_ui_refs[ch.name]['btn_rev'].configure(fg_color="#7D3C98" if ch.reverse else "#444444")

    def toggle_mute(self, ch, btn): ch.mute = not ch.mute; btn.configure(fg_color="#922B21" if ch.mute else "#444444")
    
    def toggle_rev(self, ch, btn):
        ch.reverse = not ch.reverse; btn.configure(fg_color="#7D3C98" if ch.reverse else "#444444")
    ...
    def toggle_solo(self, ch, btn):
        ...
        btn.configure(fg_color="#9C640C" if ch.solo else "#444444")
    ...
    def reset_rand_sam(self, e=None):
        self.save_state()
        for ch in self.engine.channels:
            if not ch.locked: self.handle_ch_slider(ch, 'sample', 0, True, 250)
    ...
    def change_pattern(self, val):
        self.save_state()
        target_pat, curr_pat = int(val) - 1, self.engine.current_pattern
    ...
    def on_paint(self, e, state):
        try:
            ...
            if w_str in self.widget_to_step:
                ch_idx, step_idx = self.widget_to_step[w_str]
                if self.engine.channels[ch_idx].sequence[self.engine.current_pattern][step_idx] != state:
                    self.engine.channels[ch_idx].sequence[self.engine.current_pattern][step_idx] = state
                    self.grid_buttons[(ch_idx, step_idx)].configure(fg_color=ROW_COLORS[ch_idx] if state else "transparent")
                    
            if getattr(self, 'drag_action', None) and state:
                if hasattr(self, 'widget_to_btn_action') and w_str in self.widget_to_btn_action:
                    act, ch_idx = self.widget_to_btn_action[w_str]
                    if act == self.drag_action and ch_idx not in self.drag_toggled_channels:
                        self.drag_toggled_channels.add(ch_idx)
                        ch = self.engine.channels[ch_idx]
                        btn = self.ch_ui_refs[ch.name][f'btn_{act}']
                        if act == 'mute': self.toggle_mute(ch, btn)
                        elif act == 'solo': self.toggle_solo(ch, btn)
                        elif act == 'lock': self.toggle_lock(ch, btn)
                        elif act == 'rev': self.toggle_rev(ch, btn)
        except Exception: pass
